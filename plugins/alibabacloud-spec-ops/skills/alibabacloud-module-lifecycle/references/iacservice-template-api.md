...
## 与 spec-ops 直接执行链路的边界

`alibabacloud-spec-ops` 内有两条 Terraform 执行/复用链路，状态模型不同：

| 场景 | 入口 | 状态主键 | 适用 |
| --- | --- | --- | --- |
| 临时 HCL / POC 执行 | MCP Server Core `AlibabaCloud___RunIaC` (`plan`/`apply`/`destroy`) | `state.last_process_id` 等 RunIaC process 字段 | 一次性验证、Day-2 增量修改 |
| 可复用模板 / Module 运行 | `modules -> tasks -> jobs` | `module.module_id`, `module.task_id`, `module.job_id` | 标准模板发现、发布、复用、持续维护 |

不要把 RunIaC `processID` 当作 IaCService template task 的替代，也不要把 task/job
当作现有 ad hoc 项目的远程 state。POC 验证成熟后，应通过 Module 生命周期显式晋升为
可版本化模板，再用 task/job 复用。
...
如果模板内还会访问 ECS、VPC、KMS、RAM 等产品，调用身份还需要拥有模板实际创建资源所需
的产品权限。模板发布权限和模板运行权限是两类权限，排查时要分开看。

## CLI 命令格式

`aliyun` CLI 直接接受规范 API 名：`aliyun iacservice ListModules ...`。
下面使用的 kebab-case 别名与 `alibabacloud-spec-ops` 已有风格一致
（例如 `list-modules`、`create-job`、`get-execute-state`）。如果某个别名被拒绝，
...
CallCLI 运行在远端，不能读取本地仓库路径。若 Module 仍只存在本地文件中，必须先发布到
IaCService 支持的远端来源，或使用 CLI 当前版本已验证支持的 Upload/Editor 载荷参数。
不要传 `file://`、本地绝对路径、`$(cat ...)` 或 shell 管道。
...
- 一次性直接执行（Terraform plan/apply/destroy）由
  `alibabacloud-executing-plans` 通过 MCP Server Core `AlibabaCloud___RunIaC`
  覆盖；临时 HCL 用它，持久模板用 `alibabacloud-module-lifecycle`。

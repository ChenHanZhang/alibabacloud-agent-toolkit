...
`alibabacloud-spec-ops` 内有两种 IaCService 用法，状态模型不同：

| 场景 | 入口 | 状态主键 | 适用 |
| --- | --- | --- | --- |
| 临时 HCL / POC 执行 | `execute-terraform-plan/apply/destroy` | `state.state_id` | 一次性验证、Day-2 增量修改 |
| 可复用模板 / Module 运行 | `modules -> tasks -> jobs` | `module.module_id`, `module.task_id`, `module.job_id` | 标准模板发现、发布、复用、持续维护 |

不要把 `state_id` 当作 IaCService template task 的替代，也不要把 task/job
...
- 一次性直接执行（`execute-terraform-plan/apply/destroy`）由
  `alibabacloud-executing-plans` 覆盖；临时 HCL 用它，持久模板用
  `alibabacloud-module-lifecycle`。
